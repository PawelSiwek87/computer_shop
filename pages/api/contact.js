...
const generateEmailContent = (data) => {
  ...
  const htmlData = Object.entries(data).reduce((str, [key, val]) => {
    return (str +=
      key != "password"
        ? `<h3 class="form-heading" align="left">${CONTACT_MESSAGE_FIELDS[key]}</h3><p class="form-answer" align="left">${val}</p>`
        : "");
  }, "");
...
async function handler(req, res) {
  let isError = false;
  sgMail.setApiKey(process.env.SENDGRID_API_KEY); //dodany
  //console.log(req.body);
  const data = req.body; 
  const msg = {
    to: data.email,//to,
    from: process.env.SENDGRID_HOST_MAIL,//from,
  ...
  res.status(200).json({ isError: isError });
