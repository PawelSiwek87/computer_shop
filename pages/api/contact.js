...
const generateEmailContent = (data) => {
  const stringData = Object.entries(data).reduce(  //usunąć wyświetlanie hasła
    (str, [key, val]) =>
      ...
      ( str += (key != 'password') ? `${CONTACT_MESSAGE_FIELDS[key]}: \n${val} \n \n` : '' ),
    ""
  ...
  const accountActivationToken = jwt.sign(data, process.env.JWT_PRIVATE_KEY, {expiresIn: '20m'} ); //dodać tu zawartość konta  {expiresIn: '20m'}
  
  const activationLink = process.env.DOMAIN_LINK + "/accountActivation/" + accountActivationToken;
