...
export const StateContext = ({ children }) => {
  ...
  //const [productsToPurchased, setProductsToPurchased] = useState([productsToPurchased]);

  const [selectedCategory, setSelectedCategory] = useState(""); {/* do edycja w useStateContext  */}
  const [fromPrice, setFromPrice] = useState(null); {/* do edycja w useStateContext  */}
  ...
  //console.log("UserHistoryCartItem: ");
  //console.log(userHistoryCartItems);
      //return {
  ...
  //getServerSideProps().then(function(result) {
    ...
    //console.log('purchasedProduct po then:', result.props.purchasedProduct);
    //setUserHistoryCartItems(result.props.purchasedProduct);
	//});

  //console.log("purchasedProduct po useEffec - userHistoryCartItems");
  ...
  const toggleCartItemQuanitity = async (id, value) => {
    ...
    if(value === 'inc') {
      ...
      //setCartItemsToStorage([...otherCartItems, { ...foundProduct, quantity: foundProduct.quantity + 1 } ]);
      //setCartItemsToStorage([...otherCartItems, { ...foundProduct } ]);
      ...
      if (foundProduct.quantity > 1) {
        cartItems[index].quantity--;
        //setCartItems(cartItems);
        //setCartItems([...otherCartItems, { ...foundProduct, quantity: foundProduct.quantity - 1 } ]);
        setTotalPrice((prevTotalPrice) => prevTotalPrice - foundProduct.price)
        setTotalQuantities(prevTotalQuantities => prevTotalQuantities - 1)
        //setCartItemsToStorage([...otherCartItems, { ...foundProduct, quantity: foundProduct.quantity - 1 } ], totalQuantities); 
